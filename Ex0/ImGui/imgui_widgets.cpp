...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect text_bb(window->DC.CursorPos + ImVec2(0, style.FramePadding.y - 1), window->DC.CursorPos + ImVec2(0, style.FramePadding.y) + label_size);
    if (label_size.x > 0)
    {
    ...
    if (*v)
    {
        ...
        const float pad = ImMax(1.0f, (float)(int)(check_sz / 9.5f));
